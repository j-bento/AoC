...
complete=[False for i in range(len(G))]
lC=int()
while not all(complete) and tirages:
    tires.append(tirages[0])
    tirages.remove(tirages[0])
    for i in range(len(G)):
        if verif(G[i],tires) and not complete[i]:
            complete[i]=True
            lC=i #dernière grille complétée

nontires=sum([j for j in G[lC] if j not in tires])
